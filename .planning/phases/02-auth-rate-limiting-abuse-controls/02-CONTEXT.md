...
- **D-11:** Disposable-email heuristic: **soft-flag** signups where the email domain matches a curated blocklist (seed from `disposable-email-domains` npm list, cache in Redis 24h TTL). Flagged signups are NOT rejected — they are routed to a harder Turnstile challenge (managed → interactive) and their per-email signup limit is tightened to 1/7d. Hard-reject ONLY if the domain appears on a stricter "known-throwaway" sublist (10minutemail, mailinator, guerrillamail, etc.).
  - **Rationale:** Hard-reject would block legitimate privacy-focused users (proton.me, tuta.io, fastmail subdomains) — these domains sometimes land on overly aggressive lists. Soft-flag + extra challenge raises attacker cost without collateral damage. **User override of auto-default D-11 (2026-04-15).**
