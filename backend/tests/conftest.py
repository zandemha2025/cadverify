...
def box_step_bytes() -> bytes:
    """A real single-solid STEP file (20mm box) generated by gmsh's OCC kernel.

    Skips the dependent test cleanly when gmsh is not installed, so the suite
    stays green on a no-gmsh CI runner. No binary blob is committed — the STEP
    is synthesized at test time, exactly mirroring the upload path's input.
    """
