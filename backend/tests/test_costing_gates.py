"""Gate tests G1-G7 on internal model-calibration fixtures (spec §12).

The suite is generated deterministically from repository-authored primitives, so
protected CI exercises every gate without redistributing third-party CAD.

G6 validation anchor (positioning, not a precision claim — strategy §2):
    A ~67 cm³ nylon SLS mount at low volume from a service bureau commonly runs
    ~$80-180/unit. The calibration envelope is the stated positioning anchor;
    V0 does NOT claim absolute should-cost accuracy (±40-60%).
"""
...
from src.costing.calibration_fixtures import INVALID_FIXTURE_NAME
from src.costing.harness import (
    MEDIUM_FLAT_MOUNT,
    SMALL_ROTATIONAL_ADAPTER,
    ensure_fixture_parts_dir,
)

PARTS_DIR = ensure_fixture_parts_dir()

FLAT_MOUNT = MEDIUM_FLAT_MOUNT
ROTATIONAL_ADAPTER = SMALL_ROTATIONAL_ADAPTER
INVALID_OPEN_SHELL = INVALID_FIXTURE_NAME
RING = "cv_cal_tiny_rotational_ring.stl"
...
pytestmark = pytest.mark.skipif(
    not (
        ...
        and all(
            ...
            for fname in (FLAT_MOUNT, ROTATIONAL_ADAPTER, INVALID_OPEN_SHELL, RING)
        )
    ),
...
def test_g1_invalid_geometry_never_costed_across_all_parts():
    ...
    # At least the intentionally open calibration shell must be caught.
    assert invalid_count >= 1


def test_g1_open_shell_is_invalid():
    report, _ = _decide(INVALID_OPEN_SHELL)
...
def test_g2_no_turning_or_superalloy_on_bracket():
    """Flat mount (polymer, not rotational): no turning or superalloy routing."""
    report, _ = _decide(FLAT_MOUNT)
...
def test_g2_turning_only_when_rotational():
    ...
    bracket_report, _ = _decide(FLAT_MOUNT)
...
# ── G3 explainable cost ────────────────────────────────────────────────────
def test_g3_flat_mount_estimates_itemized_and_summing():
    report, _ = _decide(FLAT_MOUNT)
    assert report.estimates
    for e in report.estimates:
...
def test_g4_flat_mount_crossover_and_make_vs_buy():
    """Coherence (weakness #7) + DFM-gated headline (weakness #6)."""
    report, _ = _decide(FLAT_MOUNT, quantities=[50, 5000])
    dec = report.decision
...
def test_g4_decision_coherence_across_parts():
    """#7 across the calibration set (loop, like G1): for every OK part the
...
def test_g4_raising_tooling_moves_crossover_right():
    base, _ = _decide(FLAT_MOUNT, quantities=[50, 5000])
    bumped, _ = _decide(FLAT_MOUNT, quantities=[50, 5000],
                        rate_overrides={"tooling.INJECTION_MOLDING": 60000})
    assert bumped.decision.crossover_qty > base.decision.crossover_qty


# ── Weaknesses #1/#2 nesting, #3 min-charge (calibration parts) ──────────────
def test_nesting_reduces_powderbed_machine():
    ...
    (For the large flat mount n=16, so machine drops from the V0 $103.82
    isolated build to ~$37.50.)"""
    report, _ = _decide(FLAT_MOUNT, quantities=[50])
...
def test_min_charge_floor_calibration_part():
    ...
    report, _ = _decide(ROTATIONAL_ADAPTER, quantities=[1])
...
def test_r2_serial_xy_nesting_amortizes_sweep():
    """R2: a medium FDM part nests >=1 per plate (the rotational adapter nests many)
    ...
    assert n > 1, "rotational adapter must nest >1 FDM part per plate (XY footprint)"
...
def test_g6_sls_estimate_in_service_bureau_band():
    """Positioning anchor (V1, nested): flat-mount SLS ~$47/unit sits inside a nested
    powder-bed bureau band ~$30-110 (V0 was $126 for an un-nested isolated build)."""
    report, _ = _decide(FLAT_MOUNT, quantities=[50])
    sls = [e for e in report.estimates if e["process"] == "sls"]
    assert sls, "SLS should be costable for the flat mount"
...
def test_g7_no_outbound_socket_during_costing():
    ...
    path = os.path.join(PARTS_DIR, FLAT_MOUNT)
