...
test("API-key mutations use finite same-origin responses before refresh", async () => {
  ...
  assert.match(button, /new CustomEvent<KeyRevealDetail>\(KEY_REVEAL_EVENT/);
  ...
  assert.match(modal, /addEventListener\(KEY_REVEAL_EVENT, reveal\)/);
  assert.match(modal, /removeEventListener\(KEY_REVEAL_EVENT, reveal\)/);
