...
test("CAD_ACCEPT lists exactly the engine's CAD extensions", () => {
  assert.equal(CAD_ACCEPT, ".stl,.step,.stp");
  assert.deepEqual([...CAD_EXTS], ["stl", "step", "stp"]);
});
...
test("isSupportedCad accepts stl/step/stp in any case, rejects the rest", () => {
...
test("supportedCadLabel reads as a human list", () => {
  assert.equal(supportedCadLabel(), "STL, STEP or STP");
});
