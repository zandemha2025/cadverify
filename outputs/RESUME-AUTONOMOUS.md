...
- **B3 feat/org-membership — SECURITY FIXED (9d9a882), merge held on ONE test-hermeticity item.** The cross-tenant mirror-collision bypass is CLOSED: migration 0025 adds org_invites.invited_user_id (resolved at create), accept_invite requires accepting.id==invited_user_id with a collision-safe fallback. Proven: old guard grants attacker admin; fixed guard 403s. All 3 lenses PASS (round 0). Branch = 24-file org feature (lifecycle/invites/deactivation/audit + 2 migrations), merges CLEAN vs dev. **BUT** it introduces test_org_membership.py whose test_invite_lifecycle fails in the FULL suite (pre-existing cross-loop asyncpg pool poisoning — dev has no org test today, so merge would push 24→25 fails). **Launched wf_ab0d3770-509** to fix that at the source (anti-cheat verifier: no skip/xfail, real assertions kept, full suite back to exactly 24 env-fails). Gate + merge B3 when it returns.

## MAKE-IT-REAL: full product prototype (workflow wf_e94a9d43-724, launched ~23:45 ET)
Founder directive: "take the Claude Design design and make it real." Building the FULL product prototype (`handoff_cadverify_2026-07-04/product/Product - Verify.dc.html`, ~14 surfaces) into the production app on **feat/product-real**, every surface wired to the REAL backend (no fixtures), honest empty/IN-DEV states. Structure: Map (foundation reads the design fully, inventories each surface→real endpoint→status, builds the shared light-instrument shell) → per-surface build (build/extend, /verify+Machines+Records already exist so EXTEND) → wiring-reality + fidelity/honesty verify → **seed a REAL demo floor** (declare machines/rates, run real /validate+/validate/cost so Home/Triage/Records/Programs render POPULATED with genuine engine outputs, not mockup data) → human-sim walk + 5-bar rubric scorecard + screenshots to outputs/product-proof/.
KEY FINDING that triggered this (visually confirmed): the built /verify is a faithful port, but shown EMPTY (no seed floor) and the surrounding surfaces (Home modules, ask-dock §10, catalog/compare/triage/programs §B8) are thin/stubbed. The design's richness = demo fixtures honesty forbids shipping — so we get it back via REAL seed data.
Two other workflows also running: **wf_bb176ceb-4c5** (site-theater: the 12 dark-theater MARKETING pages, feat/site-theater) and **wf_b840a0ad-64d** (B3 org-membership cross-tenant fix).
