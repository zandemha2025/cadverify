...
---
[DESIGN] BUILT p0-refound — graphite/cobalt governed-catalog re-founding on feat/p0-refound (commit ec98897). Token swap + 4-zone shell + PartWorkspace L2 Decision frame + reframed Inspector; deleted Gen-3 cockpit shell (LivingInstrument + 6 cockpit files). Demo path (cost/analyze + CostArtifactBar + cost-decisions) preserved; provenance infrastructural, no fabricated %. tsc clean · npm test 17/17 · next build --webpack registers all routes. Impl note: outputs/impl/p0-refound-note.md

[VERIFY] p0-refound CLOSED=yes productionWorthy=yes confidence=high — demo path preserved (/analyze→PartWorkspace(mfg), /cost→PartWorkspace(design)); crossover scrubber wired to lib/breakeven (deriveBreakeven/posToQty/qtyToPos/recommendAt), live-flips at breakeven; DFM/Routing keeps AnalysisDashboard+RoutingCard+DfmMatrix; Glass Box=DriverBreakdown; Inspector(Lineage/Governance/Sources/Audit). Phase-2 artifact intact: CostArtifactBar(save/PDF/JSON/CSV/share, real endpoints) + cost-decisions list/[id]/compare + s/cost public; SavedCostDecisionView converged on DecisionHeadline+ConfidenceInterval+CostDecisionCard (DecisionReadout dropped). Honesty: no fabricated ±% (renders confidence.half_width_pct + validated verbatim); provenance MEASURED/SHOP/USER/DEFAULT, hollow DEFAULT; zero-egress badge scoped to LOCAL_PATHS[cost,analyze,cost-decisions,history,batch], reconstruct excluded. Identity: dark-first default, graphite #080b0f/cobalt #4c90f0, faceplate/hero-field/bezel/obsidian CSS marketing-only, cv-paper/cv-twilight fenced to page.tsx, Geist Mono .readout. tsc=clean(0) · npm test=17/17 pass(0) · next build --webpack=success(0), 20 routes incl /cost /analyze /cost-decisions[/id,/compare] /s/cost/[shortId]. Non-blocking: Archivo still loaded (fenced to marketing .cv-display only) w/ stale layout comment. verifier=opus-4.8
