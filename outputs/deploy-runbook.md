...
### Object storage
**There is no S3/blob object-store integration today** — despite an advertised `S3_INPUT_ENABLED` flag (see §5), there is no `boto3`/S3 client in `backend/requirements.txt`. All blobs (meshes, batch inputs, PDFs, SAM-3D cache) live on **local disk** under `BLOB_STORAGE_PATH=/data/blobs` (and its children `MESH_BLOB_DIR`, `RECON_BLOB_DIR`, `BATCH_BLOB_DIR`, `PDF_CACHE_DIR`, `SAM3D_CACHE_DIR`). This is a real, persistent volume, not optional:
...
| `S3_INPUT_ENABLED` | `false` (`"0"`) | `src/api/batch_router.py:120` | **Leave off** | Flipping it on does **not** enable real S3 fetch — no `boto3`/S3 client exists in `requirements.txt`, and the code comment (`batch_router.py:114-119`) says the coordinator "raises `NotImplementedError` per item for remote input, orphaning the batch." Turning this on today would let requests past the honest `501` guard into a code path that **creates and then orphans batches**. Wait for the W2 connectors wall. |
