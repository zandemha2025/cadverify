...
38. **Decision dead-end** — partially closed by durable RFQ/supplier evidence ZIP packages (`rfq_packages`, `/rfq-packages`) generated from saved decisions. Remaining gap is last-mile procurement: supplier registry/qualification, capex/outsourcing tracker, and live RFQ send/integration. `build` (M).
...
## H. Deploy blockers (from the deploy-runbook grounding, 2026-07-04 — see outputs/deploy-runbook.md)
45. **Self-hosted/Helm frontend image is broken as configured** — frontend/Dockerfile copies `.next/standalone` but `next.config.ts` never sets `output:"standalone"` (commented out), so `next build` won't produce it. GA/self-host deploy blocker; cheap fix but needs a real docker build to verify. `build` (S). Does NOT block the local pilot (run-local-app.sh path).
46. **dev's HEAD has never been proven green in CI** — `main` is 176 commits behind `dev`; last fully-green CI (incl. Docker Build + Fly deploy) was 2026-06-18; the suite has only been proven via local runs since. Real production hygiene gap. `build` (S — get a green CI run on dev's HEAD). `world`-adjacent (CI infra).
47. **Reconstruction (image→mesh) has no backend in the deploy topology** — torch/tsr excluded from the API image, no GPU node/worker; the honest `local` default reports `unavailable` out of the box. Expected + honest (not a core-loop blocker); flag if the product ever promises image→mesh. `external`/scope.
48. Runbook also flagged (unproven, GA): Helm PVC ReadWriteOnce vs 2+2 replicas conflict; no Prometheus ServiceMonitor; unverified gmsh shared-libs (libGLU/libXft) in the runtime image; empty SAML SP key/cert placeholders; no load/soak evidence. `build`/`external`.
   Note: MIN_REAL_RECORDS is a hardcoded constant (=8), NOT an env flag; remote S3/manifest batch references now reject with `501 S3_INPUT_UNSUPPORTED` until a real object-fetch adapter exists.

---
**FOUNDER DECISION 2026-07-04:** the first partner is Aramco-scale — **millions of parts, EVERY file type and part type.** Consequences: (1) **B4 STEP ingestion is now a CONFIRMED HARD PILOT BLOCKER** — mechanical parts are overwhelmingly STEP; STL-only means an empty pilot. Needs OpenCascade/OCP native libs on the deploy target (cadquery/OCP — "known-hard, not installable in the dev env"). This is the #1 pilot-enabling ENGINE task now. (2) "Millions of parts" = the pilot IS the triage-at-scale story (Phase D done) — batch/BOM ingest at scale, part_summary projection performance, and the "declare floor once → triage the catalog" flow matter. Scale/perf testing becomes pilot-relevant. (3) "Every part type" elevates weldments/multi-body/sheet-metal (§44/B16) + GD&T — but the honest move at scale is that TRIAGE BUCKETS what it can't yet verify (STEP-needs-OCP, multi-body, geometry-invalid) rather than needing 100% coverage day one; the pilot starts as long as triage surfaces the gaps honestly.

**B2 PROVEN (2026-07-04):** backend core loop demonstrated live on a fresh backend at dev HEAD — signup→validate→validate/cost→Phase C verification block (makeable_in_house on a real machine)→sour-service env round-trip (NACE MR0175 exclusions, verdict flipped to makeable_outsource_only)→persistence→list, ALL asserts PASS, 0 fail. Transcript: outputs/pilot-proof/backend-loop.md. STILL OPEN from B2: frontend browser screenshot + run-path patch (flag+worker into run-local-app.sh) — agent died on session limit before both.

**FOUNDER DECISION 2026-07-04 (evening) — THE DROP-AUDIT RUBRIC.** The product moment is: put a file in, the audit runs. Every human-sim walk and every UI gate now scores that moment on five bars:
1. **Cinematic** — the drop→audit→verdict walk feels like a produced sequence (staging, motion, reveal), not a form POST.
2. **Data true** — every number on screen is a real engine output (standing honesty rule; B1 validated-accuracy is the end state).
3. **Environment correct AND cinematic** — the environment door is factually right (cited exclusions, e.g. NACE) *and* part of the show, not a settings pane.
4. **Drillable** — every claim opens: provenance, per-machine lattice, findings, similar parts. Maps to §B8 wiring, §9 triage drill-down, §11. A number that dead-ends fails this bar.
5. **Conversational** — you can interrogate the audit (why, what-if) and get engine-grounded answers. Maps to §10 ask-the-engine — **elevated by this rubric from mid-queue to a first-class product bar** (deterministic v1 first: constrained grammar → EstimateOptions, honest refusal).
Consequences: (a) the human-sim E2E gate gains a rubric scorecard — each walk reports a score + evidence per bar; (b) first rubric audit of the live /verify runs as soon as the B2b run-path leaves the app up; (c) §10 and §B8 move up the GA queue (bars 4+5); (d) "cinematic" gets a design-taste pass with founder eyes on the walk — agents can flag flatness but the taste call is the founder's.

*Top of queue (re-ranked; PILOT bar first, then the drop-audit rubric): **B4 STEP ingestion** → **B3 org membership** → **B5 units landmine** → **B2b run-path** (all four IN FLIGHT, wf_965cbd62-819) → **rubric audit of live /verify** (five-bar scorecard, human-sim) → then GA re-ranked by the rubric: wire ready backends into stubbed surfaces (§B8 — rubric bar 4, drillable) → **ask-the-engine v1 (§10 — rubric bar 5, conversational; ELEVATED)** → programs-as-object (§36) → verdict process-specs (§33/B10) + shop certs (§34/B11) → decision staleness (§37) → onboarding (§17) → verification-record export (§12) → deploy fixes (§45-48) → the rest by impact. Scope calls needed from founder: §38 (execution bridge), §43 (which verticals), §44 (weldments/sheet-metal).*
