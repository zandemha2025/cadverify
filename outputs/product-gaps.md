...
*Top of queue (re-ranked; PILOT bar first): **B4 STEP ingestion (OCP on deploy target)** — now the #1 pilot engine blocker → **B3 org membership** (WIP preserved on feat/org-membership @3a9575d, tests unrun) → **B5 units landmine** (WIP on feat/cost-unit-safety @8bfe484, incomplete) → finish B2 (frontend screenshot + run-path patch) → then GA: wire ready backends into stubbed surfaces (catalog/compare/triage/portfolio/capability — backends EXIST, §B8) → programs-as-object (§36) → verdict process-specs (§33/B10) + shop certs (§34/B11) → decision staleness (§37) → ask-the-engine v1 (§10) → onboarding (§17) → verification-record export (§12) → deploy fixes (§45-48) → the rest by impact. Scope calls needed from founder: §38 (execution bridge), §43 (which verticals), §44 (weldments/sheet-metal), B4 (STL vs STEP).*
